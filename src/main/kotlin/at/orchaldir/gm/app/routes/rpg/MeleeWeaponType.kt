...
class MeleeWeaponTypeRoutes : Routes<MeleeWeaponTypeId, SortMeleeWeaponType> {
    ...
    override fun update(call: ApplicationCall, id: MeleeWeaponTypeId) = call.application.href(Edit(id))
}
...
fun Application.configureMeleeWeaponTypeRouting() {
    routing {
        ...
        get<MeleeWeaponTypeRoutes.New> {
            handleCreateElement(STORE.getState().getMeleeWeaponTypeStorage()) { id ->
                MeleeWeaponTypeRoutes.Edit(id)
            }
        }
        get<MeleeWeaponTypeRoutes.Delete> { delete ->
            handleDeleteElement(delete.id, MeleeWeaponTypeRoutes.All())
        }
