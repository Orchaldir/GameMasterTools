...
class CurrencyUnitRoutes : Routes<CurrencyUnitId, SortCurrencyUnit> {
    ...
    override fun gallery(call: ApplicationCall, sort: SortCurrencyUnit) = call.application.href(Gallery(sort))
...
fun Application.configureCurrencyUnitRouting() {
    routing {
        ...
        get<CurrencyUnitRoutes.Gallery> { gallery ->
            val state = STORE.getState()
            val routes = CurrencyUnitRoutes()
            val units = state.sortCurrencyUnits(gallery.sort)
