...
fun State.canDeleteJob(job: JobId) = DeleteResult(job)
    ...
    .addElements(getPreviousEmployees(job))
    .addElements(getDomainsAssociatedWith(job))
...
fun State.getJobs(statistic: StatisticId) = getJobStorage()
    .getAll()
    .filter { it.importantStatistics.contains(statistic) }
