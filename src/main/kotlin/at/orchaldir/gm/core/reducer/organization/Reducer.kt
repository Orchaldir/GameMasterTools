...
import at.orchaldir.gm.core.reducer.team.CREATE_TEAM
import at.orchaldir.gm.core.reducer.team.DELETE_TEAM
import at.orchaldir.gm.core.reducer.team.UPDATE_TEAM
...
val ORGANIZATION_REDUCER: Reducer<OrganizationAction, State> = { state, action ->
    when (action) {
        ...
        // team
        is CreateTeam -> CREATE_TEAM(state, action)
        is DeleteTeam -> DELETE_TEAM(state, action)
        is UpdateTeam -> UPDATE_TEAM(state, action)
    }
}
