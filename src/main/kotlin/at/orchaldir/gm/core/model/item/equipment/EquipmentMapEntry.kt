...
@Serializable
data class EquipmentMapEntry<T>(val data: T, val sets: Set<Set<BodySlot>>) {
    ...
    companion object {
        ...
        fun <T> from(value: T, data: EquipmentAppearance) = from(value, data.slots().getAllBodySlotCombinations().first())
    ...
    fun <U> convert(function: (T) -> U): EquipmentMapEntry<U> = EquipmentMapEntry(
        function(data),
        sets,
    )
