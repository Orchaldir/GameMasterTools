...
import at.orchaldir.gm.core.model.character.PERSONALITY_TRAIT
...
fun loadStorage(path: String, type: String) = when (type) {
    CALENDAR -> loadStorage(path, CalendarId(0), type)
    CHARACTER -> loadStorage(path, CharacterId(0), type)
    CULTURE -> loadStorage(path, CultureId(0), type)
    FASHION -> loadStorage(path, FashionId(0), type)
    ITEM_TEMPLATE -> loadStorage(path, ItemTemplateId(0), type)
    LANGUAGE -> loadStorage(path, LanguageId(0), type)
    MATERIAL -> loadStorage(path, MaterialId(0), type)
    NAME_LIST -> loadStorage(path, NameListId(0), type)
    PERSONALITY_TRAIT -> loadStorage(path, PersonalityTraitId(0), type)
    RACE -> loadStorage(path, RaceId(0), type)
    else -> throw IllegalArgumentException("Unknown type $type")
}
