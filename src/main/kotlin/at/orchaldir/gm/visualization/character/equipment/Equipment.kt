    ...
    state.equipped.getEquipmentWithSlotSets().forEach { (pair, sets) ->
        sets.forEach { set ->
            ...
            when (val data = pair.first) {
                ...
                is OneHandedClub -> visualizeClub(newState, body, data.head, data.shaft, data.fixation, true, set)
                is TwoHandedClub -> visualizeClub(newState, body, data.head, data.shaft, data.fixation, false, set)
