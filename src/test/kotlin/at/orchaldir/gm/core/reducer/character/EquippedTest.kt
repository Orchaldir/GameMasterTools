...
class EquippedTest {
    ...
    fun `Update equipment`() {
        validateEquipped(state, equippedEquipment)
    }
    ...
    fun `Cannot use unknown equipment`() {
        ...
        assertIllegalArgument("Requires unknown Equipment 0!") { validateEquipped(state, equippedEquipment) }
    }
    ...
    fun `Cannot use unknown color scheme`() {
        ...
        assertIllegalArgument("Requires unknown Color Scheme 0!") { validateEquipped(state, equippedEquipment) }
    }
    ...
    fun `Cannot use equipment with wrong slots`() {
        ...
        assertIllegalArgument("Equipment 0 uses wrong slots!") { validateEquipped(state, equippedEquipment) }
    }
    ...
    fun `Cannot occupy equipment slot twice`() {
        ...
        assertIllegalArgument("Body slot InnerTop is occupied multiple times!") { validateEquipped(state, EquippedEquipment(equipmentMap)) }
    }
}