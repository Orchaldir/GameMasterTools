...
class PopulationTest {
    ...
    inner class TotalPopulationTest {
        ...
        @Test
        fun `The total population must be greater or equal 0`() {
            assertTotalPopulation(PopulationWithSet(-1))
        }

        @Test
        fun `With an unknown culture`() {
            assertPopulation(
                PopulationWithSet(100, cultures = setOf(UNKNOWN_CULTURE_ID)),
                "Requires unknown Culture 99!",
            )
        }
        ...
        fun `With an unknown race`() {
            assertPopulation(
                PopulationWithSet(100, setOf(UNKNOWN_RACE_ID)),
                "Requires unknown Race 99!",
            )
        }
        ...
        fun `With an unknown standard of living`() {
            assertPopulation(
                PopulationWithSet(100, income = income),
                "Requires unknown Standard Of Living 99!",
            )
        }
        ...
        @Test
        fun `A valid population`() {
            validatePopulation(state, PopulationWithSet(100, races = setOf(RACE_ID_0)))
        }
